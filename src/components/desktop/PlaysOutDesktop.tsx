...
const CARDS = [
  {
    ...
    expandedBody:
      "You work with our pool of active startups, tangible materials, and real timelines (not case studies or simulations).\nYou evaluate opportunities as they unfold, discuss tradeoffs with peers, and co-invest in live deals.",
  },
  {
    ...
    expandedBody:
      "You challenge assumptions, weigh perspectives, and pressure-test decisions alongside a diverse group of thoughtful investors, in real time.",
  },
  {
    ...
    expandedBody:
      "You move from debate to action by co-investing with peers, pooling judgment, sharing responsibility, and committing capital together.",
  },
  {
    ...
    expandedBody:
      "Through repeated real decisions, you emerge with a personal investment perspective: a repeatable way to assess risk, quality, and fit on your own terms.",
  },
];
...
>(({ card, index, imageRight }, ref) => {
  ...
  const handleScroll = useCallback(() => {
    ...
    const nextSibling = stickyDiv.nextElementSibling as HTMLElement | null;
    if (!nextSibling || nextSibling.tagName !== "DIV") {
      setOpacity(1);
      return;
    }

    const nextStickyTop = TITLE_OFFSET + (index + 1) * STACK_OFFSET;
    const nextRect = nextSibling.getBoundingClientRect();
    const nextDistanceFromStuck = nextRect.top - nextStickyTop;

    const fadeStartDistance = window.innerHeight * 0.4;

    if (nextDistanceFromStuck <= 0) {
      setOpacity(0.4);
    } else if (nextDistanceFromStuck < fadeStartDistance) {
      const progress = 1 - nextDistanceFromStuck / fadeStartDistance;
  ...
  const textContainerJSX = (
    <div
      ...
      style={{
        ...
        borderRadius: 40,
        boxShadow:
          "0 25px 50px -12px rgba(0, 0, 0, 0.5), 0 -30px 60px -15px rgba(0, 0, 0, 0.3)",
      }}
    >
      ...
      <div className="absolute bottom-10 right-10 z-30 transition-all duration-300 group-hover/card:scale-110">
        <motion.span
      ...
      >
        <div className="flex items-center gap-5">
          <div
            ...
            style={{ background: "rgba(7,12,23,0.12)" }}
          />
          <p
